use crate::eval::{Expr, Procedure,  SymbolTable};
    ...
    fn stringify(&self, symbol_table: &SymbolTable) -> String {
        match self {
            ...
            Expr::Call(head, tail) => {
                ...
                let tail_str = tail.iter()
                .fold(String::new(), |mut buf, expr| {
                    buf.push(' ');
...
impl Stringify for Procedure {
    fn stringify(&self, symbol_table: &SymbolTable) -> String {
        let args_str = self.params().iter()
        .fold(String::new(), |mut buf, symbol| {
            buf.push(' ');
